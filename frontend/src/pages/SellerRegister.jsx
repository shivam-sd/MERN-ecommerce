...
const SellerRegister = () => {
  ...
  const [businessType , setBussiness] = useState(""); 
  ...
  const handleFormData = async (e) => {
    ...
    if (!name || !email || !phone || !businessType || !password || !confirmPassword) {
    ...
    try {
      console.log("Seller Data Register", sellerData);
      const response = await axios.post(
        `${import.meta.env.VITE_USERS_BASE_URL}/seller/register`,
        sellerData,
        {
          headers: {
...
<label className="mt-3 text-xl" htmlFor="businessType">Business Type:</label>
          <input
            className="mt-1 lg:w-80 md:w-80 h-8 rounded-sm text-lg pl-1 outline-none focus-within:shadow-md border border-slate-400"
            type="text"
            id="businessType"
